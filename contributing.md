...
If you would like to help out the project by developing new features or fixing bugs yourself. You can do so by forking this repository, making your changes and opening a pull request in this repository to merge your fork back into this repo.
...
1. Install the project dependencies by running `npm i` within the project directory.
1. Build and launch the app in development mode by running `npm run dev`.
...
Packaging the app for distribution is done using [electron-builder](https://www.electron.build/). Running `npm run package` will create packages for most platform/architecture combinations and output them to the `dist` directory.