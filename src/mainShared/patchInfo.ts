...
import { type DataHunk, dataHunkFrom } from "@shared/romUtils/dataHunk"
import { bankAddressOfROMOffset, farcallDataFormat, markROMBytesAsUnused, markROMBytesAsUsed, markROMBytesInBankAsUsed, returnInstructionDataFormat, type ROMInfo, romOffsetFromBankAddress } from "@shared/romUtils/romInfo"
...
export class PatchInfo {
  ...
  readonly hunks = (romInfo: ROMInfo): DataHunk[] => {
    this.freeLocations.forEach((location) => {
      markROMBytesAsUnused(romInfo, location.offset, location.size)
    })
    ...
    hunkFormats.forEach((hunkFormat) => {
      Object.entries(hunkFormat.dataFormat.referenceOffsets).map(([key, value]) => {
        referenceAddresses[key] = bankAddressOfROMOffset(hunkFormat.romOffsets[0]) + value
      })
    })
    
    return hunkFormats.flatMap((hunkFormat) => {
      return hunkFormat.romOffsets.map((romOffset) => {
        return dataHunkFrom(romOffset, hunkFormat.dataFormat, referenceAddresses)
      })
    })
  }
  
  readonly hunkFormatFrom = (romInfo: ROMInfo, changeInfo: ChangeInfo): HunkFormat[] => {
    const romOffsets: number[] = []
      ...
      if (location instanceof LocationInfo) {
        if (isNotNullish(location.maxSize)) {
          ...
          if (location.maxSize < changeSize) {
            const newOffset = markROMBytesAsUsed(romInfo, dataFormat.size())
            const farcall = farcallDataFormat(newOffset)
            romOffsets.push(newOffset)
            
            if (location.freeUnused) {
              farcall.add(returnInstructionDataFormat())
            } else {
              farcall.add(new DataFormat(Array(location.maxSize - farcall.size()).fill(0)))
              dataFormat.add(returnInstructionDataFormat())
            }
          ...
          if (location.freeUnused) {
            markROMBytesAsUnused(romInfo, location.offset + changeSize, location.maxSize - changeSize)
          }
        } else {
          romOffsets.push(location.offset)
        }
      } else {
        romOffsets.push(markROMBytesInBankAsUsed(romInfo, location, dataFormat.size()))
      }
    })
...
class LocationInfo {
  ...
  constructor(object: any) {
    this.offset = romOffsetFromBankAddress(object.bank, object.address)
