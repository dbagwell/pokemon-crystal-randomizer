...
import { romOffsetFromBankAddress } from "@shared/romUtils/romInfo"
import type { MapObjectEvent } from "@shared/types/gameData/mapObjectEvent"
import { bytesFrom, isNotNullish } from "@shared/utils"

export const dataHunkFromMapObjectEvent = (event: MapObjectEvent): DataHunk => {
  return {
    offset: romOffsetFromBankAddress(event.romOffset[0], event.romOffset[1]),
    values: bytesFromMapObjectEvent(event),
  }
}
